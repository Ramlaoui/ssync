        ...
        >
          <div class="flex justify-between items-start mb-2">
            <div class="flex flex-col">
              <span class="text-base font-semibold text-gray-700">{job.job_id}</span>
              <span class="text-xs text-gray-500">{job.user || 'N/A'}</span>
            </div>
            <span 
          ...
          <h4 class="text-sm font-medium text-gray-800 mb-2" title={job.name}>
    ...
    <div class="overflow-x-auto">
      <table class="w-full">
        <thead class="bg-gray-50 border-b border-gray-200">
          <tr>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Job ID</th>
            ...
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Runtime</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Resources</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider hidden xl:table-cell">Partition</th>
            <th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider hidden xl:table-cell">Submitted</th>
          </tr>
        </thead>
        <tbody class="bg-white divide-y divide-gray-200">
          {#each jobs as job}
            ...
            >
              ...
              <td class="px-4 py-3">
                <div class="text-sm font-medium text-gray-700" title={job.name}>
                ...
                {#if job.cpus || job.memory || job.nodes}
                  <div class="space-y-1">
                    ...
                    {#if job.cpus && job.cpus !== 'N/A'}
                      <div class="flex gap-2">
                        <span class="text-gray-500 text-xs">CPUs:</span>
                        <span class="font-medium text-xs">{job.cpus}</span>
                      </div>
                    {/if}
                    {#if job.memory && job.memory !== 'N/A'}
                      <div class="flex gap-2">
                        <span class="text-gray-500 text-xs">Mem:</span>
              ...
              <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600 hidden xl:table-cell">
                {job.partition || 'N/A'}
              </td>
              <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600 hidden xl:table-cell">
