<script lang="ts">
  ...
  import { Download, Trash2, FileText } from 'lucide-svelte';
      ...
      <div class="template-list">
        {#each scriptTemplates as template}
          <div
            ...
            onkeydown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectTemplate(template); } }}
            role="button"
            tabindex="0"
          >
            <div class="template-item-header">
              <h4 class="template-item-name">{template.name}</h4>
              <div class="template-item-actions" onclick={stopPropagation(bubble('click'))} onkeydown={() => {}} role="presentation">
                <button
